/*
    1. 함수타입
     - 함수의 매개변수와 반환값에 타입을 저장할 수있다
     - 지정된 타입과 다른 값이 전달되거나, 반환되면 컴파일 에러가 발샹한다
 */
...
/*
    2. void
     ...
     - 반환값이 없는 함수의 기본 타입으로 사용됨
     - 계층 구조상 undefined의 super타입
*/
...
/*
    3. optional Parameter
     ...
     - 변수명?:type
*/
function fnOptonal(a:number, b?:number){
    console.log(b);
    /*
        b는 선택적 파라미터므로 타임스크립트는 b에 값이 올 수도 있고 안올 수도 있으므로
        type | undefined로 추론한다
        따라서 b를 그대로 반환시키고자 한다면 컴파일 에러가 발생할 수 있으며,
...
/*
    4. restParameter
     - 함수의 매개변수에 들어갈 인자의 수가 정해지지 않은 경우 사용하는 문법
...
/*
    ...
    일반함수에서 스프레드문법을 통해 인자를 전달할 수 없다
...
objectDesFn({a:1, b:'xogud', c:true});
...
objectDesFn2({a:111, b:'xogud', c:false})

/*
    7. 타입 내로잉
     - union type처럼 변수가 여러타입을 갖고 있을 때, 이를 사용하려고 하는
     "한가지 타입"으로 좁히는 문법
...
/*
    8. Type Assertion
     ...
     알고있는 경우 사용하는 문법으로, 컴파일러에게 "이 변수의 타입은 xx다"라고
     단언하는 문법
...
/*
    9. never type
     - 어떤 값도 가질 수 없는 타입으로, 절대 값을 반환할 수 없는 함수나
...
type ect = string|number;
function typeNarrowing2(sOrn:ect):void{
    if(typeof sOrn === 'number'){
        ...
        // 시킴으로서 모든 타입이 철저히 검사되고 있는지 확인할 수 있다

        // 타입체크용 메서드
        // 컴파일 단계의 타입체크기능을 무효화하여 check에 string, number와 같은
        // 다른 값이 전달될 경우, 런타임시 에러가 발생할 수 있도록 처리하는 함수
...
/*
    10. 함수 타입 표현식/ 함수 시그니처
     ...
     - 함수의 시그니처 
        - 함수가 가지는 매개변수으 개수와 자료형, 반환형을 합친 것.
*/
// 일반함수
