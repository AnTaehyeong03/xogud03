...
num = 12345;
// num = '12345';
num = num * Number('1'); // 타입스크립트에서는 자동형변환 사용 불가
...
// 객체 타입변수
// {key:type}
let animal:{name:string, age:number}; // 모두 지정해야
animal = {name:'coco', age:2};        // 모두 사용 가능

/*
    2. 타입추론
     - 변수에 대입하는 값을 바탕으로 자동으로 타입을 추론하는 기능
     - 간단한 변수 선언시에는 타입주석을 작성하지 않고 타입추론을 이용한다
...
/*
    3. 배열과 튜플
    배열의 특징
     - 배열의 크기에 대한 제약이 존재하지 않으며, js기준 들어가는 타입에 대한 제한도 없다
...
/*
    튜플
     - 튜플은 배열의 특수한 형태로 길이와 타입이 고정된 배열
...
/*
    4. readonly
     ...
     - 단, 튜플은 javascript에는 존재하지 않는 타입으로, 컴파일 완료 후에는 배열로 반환된다
     - 타입스크립트의 정적타입체크 시스템은 실제 코드를 "실행하기 전" 단계에서 작동하므로,
     실행 환경에서 동작하는 "메서드에 대한 검사"는 수행하지 않는다.
     - 이 경우 튜플에 대한 논리적인 오류가 발생할 수 있는데, 이를 정제할 수 있는게 readonly
*/
numTuple.push(4);
...
//readonlyObject.name = 'xogud';

/*
    5. any Type
     - 어떤 타입이든 저장할 수 있는 타입
     - 타입스크립트의 정적타입검사를 우회하기 위해 설계되었다
     - javascript코드와의 호환성 유지 및 마이그레이션을 돕기위한 타입이고, 무분별하게 사용시
...
/*
    6. unknown type
     - 어떤 타입이든 할당할 수 있는 타입
     - 단, 타입검사기능이 동작하여, any와 달리 타입 안정성 유지 가능
*/
let unknown:unknown;
unknown = 1;
unknown = "xogud";
...
// 단순히 값을 받기위한 용도로만 사용하며, unknown에 할당된 데이터를 재할당하기 위해서는
// 적절한 타입으로 narrowing필요
//str = unknown;
//num2 = unknown;
...
let payInfo = {name:'kh풍뎅이', price:15000, method:'bank'} as const;
