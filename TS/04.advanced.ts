/*
    1. Class
     - javascript의 class문법을 보다 확장하여, 
     좀 더 객체지향적인 프로그래밍이 가능하도록 만들었다.
...
/*
    3. Generics
     ...
     타입변수라고 부른다
     - Type에 유연성을 제공하면서 확장성과 타입안정성을 추가할 수 있다.
     - Generics의 주요 기능
...
function func<T extends Animal>(animal: T){//명목적 타이핑으로 보이지만 구조적임
    animal.bark();
}
type Timer = {
    bark(): void;
}
// TypeScript는 구조중심의 타입검사 시스템을 사용하기 때문에 다음과 같이 작성 가능
...
function config<T extends Options = {theme:"dark"}>(option?:T) : T{
    ...
    //return option || {theme:"dark"} as T; 권장되지 않음
    // falsy? 값이 비어있으면 false

    //널병합연산자 ??
    // - a ?? b -> a가 null 또는 undefined인 경우 B반환, 아닌경우 A반환
...
function getValue<Person2, K extends keyof Person2>(person:Person2, key:K): Person2[K]{
    return person[key];
}
const person:Person2 = {name:'xogud', age:26, location:'seoul'};
...
/*
    ...
    1) Partial<T>
     ...
     - 모든속성을 필수, 읽기 전용으로 만들어주는 Required<T>, Readonly<T>도 존재한다.
*/
interface User {
    id: number
    name: string
...
// 3) Omit<T, K>
//  - 객체타입에서 특정 속성을 제외하고 나머지만 남긴 채 타입을 만들 때 사용
// id를 제외한 사용자 정보
type NewUser = Omit<User, "id">;
const userData:NewUser = {
    name: "xogud",
    email: "ta0313@naver.com"
}
/*
    Utility Type 조합
...
*/
function getUser(){
    return {id: 1, name:"xogud", email:"abc"};
}
type UserData = ReturnType<typeof getUser>;
const data:UserData = {id:11, name:'sim', email:'dd'};

/*
    6) Parameters<T>
     - 합수타입 T의 매개변수 타입을 배열로 추출
