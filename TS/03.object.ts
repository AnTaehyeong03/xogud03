...
const person1:Person = {name:'xogud', age:26};
const person2:Person = {name:'xogud2', age:27};
...
/*
    3. interface와 type aliases의 차이점
     - 두 문법의 최종적인 동작방시은 비슷하나, "확장성"에서 차이점이 있다.
     - 확장성
       - 객체의 구조를 변경하는 방법 
*/
...
/*
    ...
    2) type별칭이 적합한 경우
     - 유니온, 교차타입 등 복합 타입정의가 필요한 경우
     - 튜플, 기본타입, 함수타입에 별칭을 정의하고자 하는 경우
     - 조건부타입 및 제네릭으로 지정히고 싶은 경우
*/
...
// 인덱스 시그니처 사용시 주의점
//  - 속성에 여러 값들이 들어가는 경우, 
//    인덱스 시그니처는 그 모든 값들에 대해 표현할 수 있다
...
/*
    6. 타입호환성
     - 타입스크립트에서 두 타입이 서로 대입가능한지 판단하는 개념
     - 타입검사 매커니즘상 호환되지 않는 경우 컴파일 에러가 발생한다
     - 타입스크립트는 호환성 여부를 체크할 때 "구조적 타이핑"에 기반하여 호환가능 여부를 판단한다
...
const mkm1:Teacher = {name:'mkm', age:20};
const mkm2:Student = mkm1; // 두 변수의 타입은 달라도, 구조가 똑같기 때문에 호환

const sim = {name:'sim', age:4, location:'seoul'};
const eunseong:Student = sim; // location속성은 Student에 없지만
// 나머지 구조가 일치하기 때문에 호환된다.
// 단, 객체 리터럴을 대입하는 경우 "잉여 속성 검사"가 수행되면서 컴파일 에러가 발생

export default eunseong;
