/*
    1. 화살표 험수
     - 함수를 간결하게 표기하는 방법
     - function 키워드 없이 (매개변수) => 본문으로 작성한다
     - 일반함수와 this바인딩 방식이 다르다
*/
function declearFn(a, b){
    return a*b;
}
// 화살표 함수
const declearFn1 = (a,b) => {return a*b};
const declearFn2 = (a,b) => a*b;
const declearFn3 = a => a*1;

// 2. this바인딩
//  - this는 모든 함수에 묵시적으로 존재하는 예약어
//  - 함수를 호출한 객체의 주소값이 바인딩 되도록 설계되어있다.
function callableFn(){
    console.log(this);
}
callableFn(); // global
...
// 3. 화살표 함수의 this
//  - 화살표 함수는 자신만의 this를 가질수 없도록 설계되어있다
//  - 화살표 함수는 선언된 위치의 상위 스코프의 this를 그대로 참조하여 사용
const arrowFn = () => {
    console.log(this); // global의 this
};
arrowFn();
const obj2 = {
    name : '객체 2',
...
/*
    2) 일반함수를 사용하는게 적합한 경우
     - this가 동적으로 바인딩되어야 하는 겨우(객체의 매서드나 생성자 함수)
...
printName.apply({name:'xogud'}, ["Hello", "Mr. Lee"]);

// 3) bind(thisArg)
//  - 함수를 실행하지 않고, this가 반인딩 된 새로운 함수를 반환
