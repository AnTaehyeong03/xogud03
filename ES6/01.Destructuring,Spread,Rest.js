...
// 배열 비구조화 문법
// 배열의 인덱스 순서에 맞게 변수를 준비해두면, 각 배열에 담긴 값이 순서에 맞게 저장
const [d,e,f] = arr;
console.log(d,e,f);
...
const {bao, option = ""} = obj;
console.log(bao, option); // option에는 undefined
...
const copy_arr = [0, ...arr2, 5];
console.log(copy_arr);

// 객체 전개
const person = {name: 'xogud', job : 'student'};
const xogud = {...person , location: 'pyeongtaek'};
console.log(xogud);
...
testRest('xogud', 'qhdks', 'thwywpstps', 1, 2, 3, 4);
testRest(...arr2);

// 비구조화 할당과 Rest Parameter
function testRest2({name, age, hobby, location}){
    console.log(name, age, hobby, location);
}
const person2 = {name: 'xogud', age: 26, hobby: ['game', 'music'], location: 'pyeongtaek'};
testRest2(person2);
